...
export default function CompanyAnalytics({ token }: { token: string }) {
  ...
  const loadAnalytics = async (nextFilters = filters) => {
    ...
    try {
      ...
      const [statsResponse, usersResponse, abcResponse] = await Promise.all([
        axios.get(`${API_URL}/users/company/analytics/stats`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`${API_URL}/users/company/analytics/users`, { headers: { Authorization: `Bearer ${token}` }, params }),
        axios.get(`${API_URL}/users/company/analytics/abc`, { headers: { Authorization: `Bearer ${token}` }, params }),
      ])
      ...
      console.error(err)
      setMessage(`❌ ${err.response?.data?.message || 'Не удалось загрузить аналитику компании'}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const exportMonthlyReport = async () => {
    if (!filters.start || !filters.end) {
      setMessage('❌ Для Excel-выгрузки выбери период: дату начала и дату окончания')
      return
    }
    ...
    setMessage('')
    try {
      const response = await axios.get(`${API_URL}/users/company/analytics/export/monthly`, {
      ...
      link.download = match?.[1] || `company-report-${filters.start}-${filters.end}.xlsx`
      ...
      console.error(err)
      setMessage(`❌ ${err.response?.data?.message || 'Не удалось скачать Excel-отчет'}`)
    } finally {
      setExporting(false)
    }
  }
    ...
    <div style={{ display: 'grid', gap: 24 }}>
      <div style={{ ...cardStyle, background: 'linear-gradient(135deg, #ffffff 0%, #f5f8ff 100%)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div>
            <h2 style={sectionTitleStyle}>Аналитика компании</h2>
            <p style={{ color: '#64748b', margin: 0, maxWidth: 780 }}>
              Здесь координатор видит аналитику только по своей компании: активность сотрудников, популярные блюда и ABC-разбор.
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            ...
            <button onClick={exportMonthlyReport} disabled={exporting} style={{ background: '#198754', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 16px', height: 40, marginTop: 18 }}>
              {exporting ? 'Готовлю Excel...' : 'Скачать Excel для payroll'}
            </button>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14 }}>
        <div style={cardStyle}><div style={{ color: '#64748b', marginBottom: 8 }}>Сотрудники</div><strong style={{ fontSize: 28 }}>{stats?.totalUsers ?? 0}</strong></div>
        ...
        <div style={cardStyle}><div style={{ color: '#64748b', marginBottom: 8 }}>Сумма</div><strong style={{ fontSize: 28 }}>{formatCurrency(stats?.totalRevenue ?? 0)}</strong></div>
      </div>
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
          <h3 style={{ margin: 0 }}>Сотрудники компании</h3>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                  <div>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
                      ...
                      <Chip background={user.status === 'DISMISSED' ? '#f8d7da' : user.status === 'VACATION' ? '#fff3cd' : '#d4edda'} color={user.status === 'DISMISSED' ? '#842029' : user.status === 'VACATION' ? '#856404' : '#155724'}>
                        {userStatusLabels[user.status] || user.status || 'Активный'}
                      </Chip>
