...
const isSubmittedWeeklyStatus = (status?: string | null) => ['CONFIRMED', 'PREPARING', 'COMPLETED'].includes(status || '')
            ...
            <div style={{ background: '#fff', padding: 16, borderRadius: 8, marginBottom: 20, boxShadow: '0 2px 4px rgba(0,0,0,0.08)' }}>
              ...
              <div style={{ display: 'grid', gap: 8 }}>
                {data.employees.filter((employee: any) => employee.hasSelection).map((employee: any) => (
                  <div key={`request-${employee.userId}`} style={{ background: isSubmittedWeeklyStatus(employee.weeklyStatus) ? '#f7faff' : '#fff8e6', border: isSubmittedWeeklyStatus(employee.weeklyStatus) ? '1px solid #d7e7ff' : '1px solid #ffe69c', borderRadius: 8, padding: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                      <strong>{employee.userName}</strong>
                      <span style={{ fontSize: 13, color: isSubmittedWeeklyStatus(employee.weeklyStatus) ? '#0d6efd' : '#fd7e14', fontWeight: 700 }}>
                        {isSubmittedWeeklyStatus(employee.weeklyStatus) ? (weeklyStatusLabels[employee.weeklyStatus] || 'В заявке') : 'Черновик'}
                      </span>
                    </div>
