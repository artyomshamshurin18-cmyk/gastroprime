...
const allowedWeeklyMenuStatuses = new Set(['DRAFT', 'PENDING', 'CONFIRMED', 'PREPARING', 'REJECTED', 'COMPLETED'])
...
export class AdminService {
  ...
  private normalizeWeeklyMenuStatus(value: unknown) {
    const normalized = normalizeText(value).toUpperCase()

    if (!allowedWeeklyMenuStatuses.has(normalized)) {
      throw new BadRequestException('Некорректный статус заявки')
    }

    return normalized
  }

  private calculateWeeklyMenuTotalAmount(selections: any[], priceMap: Map<string, number>) {
    return (selections || []).reduce((menuTotal, selection) => {
      const selectionTotal = (selection.items || []).reduce((sum: number, item: any) => {
        const price = getResolvedCompanyDishPrice(item.dish, priceMap)
        return sum + price * (Number(item.quantity) || 0)
      }, 0)

      return menuTotal + selectionTotal
    }, 0)
  }

  private serializeAdminWeeklyMenu(menu: any, priceMap: Map<string, number>) {
    const selections = (menu.selections || []).map((selection: any) => ({
      ...selection,
    ...
    return {
      ...menu,
      selections,
      totalAmount: this.calculateWeeklyMenuTotalAmount(selections, priceMap),
      companyBalance: menu.user?.company?.balance || 0,
      chargedAmount: menu.chargedAmount || 0,
      chargedAt: menu.chargedAt || null,
    }
  }
  ...
  async createPrepaymentInvoice(data: { companyId: string; amount: number; comment?: string }) {
    ...
    return this.prisma.invoice.create({
      data: {
        ...
        deviationTotal: 0,
        total: amount,
        comment: normalizeText(data.comment) || 'Счет на предоплату за услуги корпоративного питания',
        ...
        lines: {
          create: [{
            description: 'Услуги предоставления корпоративного питания',
            quantity: 1,
            unitPrice: amount,
            amount,
            deviationAmount: 0,
  ...
  async getAllWeeklyMenus(start: string, end: string) {
      ...
      include: {
        user: { 
          select: { 
            ...
            firstName: true, 
            lastName: true, 
            company: { select: { id: true, name: true, balance: true } } 
          } 
        },
        selections: { 
          include: { 
    ...
    const companyIds = Array.from(new Set(menus.map((menu) => menu.user.company?.id).filter(Boolean))) as string[]
    const companyPriceMaps = new Map<string, Map<string, number>>()

    await Promise.all(companyIds.map(async (companyId) => {
      companyPriceMaps.set(companyId, await getCompanyCategoryPriceMap(this.prisma, companyId))
    }))

    return menus.map((menu) => this.serializeAdminWeeklyMenu(menu, companyPriceMaps.get(menu.user.company?.id || '') || new Map<string, number>()))
  }

  async updateWeeklyMenuStatus(id: string, status: string) {
    const normalizedStatus = this.normalizeWeeklyMenuStatus(status)

    if (normalizedStatus === 'PREPARING') {
      return this.chargeWeeklyMenuFromBalance(id)
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (normalizedStatus === 'COMPLETED' && !existing.chargedAt && existing.status !== 'PREPARING') {
        throw new BadRequestException('Сначала спишите сумму с лицевого счёта и переведите заявку в статус «В производстве»')
      }

      if (existing.chargedAt && ['DRAFT', 'PENDING', 'CONFIRMED', 'REJECTED'].includes(normalizedStatus)) {
        throw new BadRequestException('По этой заявке уже списаны средства, вернуть её в ранний статус нельзя')
      }
  ...
  async chargeWeeklyMenuFromBalance(id: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const company = menu.user.company
      if (!company?.id) {
        throw new BadRequestException('У заявки не найдена компания для списания средств')
      }

      const priceRows = await tx.companyCategoryPrice.findMany({
      ...
      const priceMap = new Map(priceRows.map((row) => [row.categoryId, row.price]))
      const totalAmount = this.calculateWeeklyMenuTotalAmount(menu.selections, priceMap)

      if (!menu.chargedAt) {
        if ((company.balance || 0) < totalAmount) {
          throw new BadRequestException(`Недостаточно средств на лицевом счёте. Нужно ${totalAmount} ₽, доступно ${company.balance || 0} ₽`)
        }
      ...
      const updatedMenu = await tx.weeklyMenu.update({
        where: { id },
        data: {
          status: 'PREPARING',
          chargedAt: menu.chargedAt || new Date(),
          chargedAmount: menu.chargedAmount || totalAmount,
        },
        include: {
          user: {
            select: {
      ...
      return this.serializeAdminWeeklyMenu(updatedMenu, priceMap)
    })
  }
