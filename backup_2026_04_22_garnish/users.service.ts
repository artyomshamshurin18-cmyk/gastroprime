...
export class UsersService {
  ...
  private async findDailyMenuForDate(date: Date, include?: any) {
  ...
  private mainIncludesGarnish(categoryName?: string | null) {
    return this.normalizeCategoryName(categoryName).includes('с гарниром')
  }
  ...
  async getCompanyDashboard(userId: string, dateValue?: string) {
    ...
    return {
      ...
      summary: {
        ...
        draftCount: employees.filter(employee => employee.hasSelection && !['CONFIRMED', 'PREPARING', 'COMPLETED'].includes(employee.weeklyStatus || '')).length,
        confirmedCount: employees.filter(employee => employee.hasSelection && ['CONFIRMED', 'PREPARING', 'COMPLETED'].includes(employee.weeklyStatus || '')).length,
  ...
  async getCompanyScheduledRequests(coordinatorUserId: string, data?: { start?: string; end?: string }) {
    ...
    const selections = await this.prisma.daySelection.findMany({
      where: {
        ...
        weeklyMenu: {
          ...
          status: { in: ['DRAFT', 'CONFIRMED', 'PREPARING', 'COMPLETED'] },
        },
      },
      orderBy: [{ date: 'asc' }],
      include: {
    ...
    for (const selection of selections) {
      ...
      if (['CONFIRMED', 'PREPARING', 'COMPLETED'].includes(selection.weeklyMenu.status)) entry.confirmedCount += 1
  ...
  async autoFillCompanyEmployees(coordinatorUserId: string, data: { date?: string; userIds?: string[] }) {
    ...
    const dailyMenu = await this.findDailyMenuForDate(targetDate, {
      items: {
    ...
    for (const employee of missingEmployees) {
      ...
      const addSet = () => {
        ...
        if (setType === 'FULL' || setType === 'MAIN_SALAD') {
          ...
          addItem(main);
          const garnish = this.mainIncludesGarnish(main.categoryName)
            ? null
            : pickMatching(filtered, item => this.isGarnishCategory(item.categoryName), usedIds);
  ...
  async createCompanyRequest(coordinatorUserId: string, data: { date?: string }) {
    ...
    const companyPriceMap = await this.getCompanyPriceMap(context.companyId)
    const draftIds = weeklyMenus
      .filter(menu => ['DRAFT', 'PENDING', 'REJECTED'].includes(menu.status))
      .map(menu => menu.id)
  ...
  async setCompanyEmployeeSelection(coordinatorUserId: string, employeeId: string, data: { date?: string; items?: { dishId: string; quantity: number }[] }) {
    ...
    const allowedByDishId = new Map<string, number>(
      dailyMenu.items.map((item: any) => [item.dishId, item.maxQuantity] as [string, number])
    )
